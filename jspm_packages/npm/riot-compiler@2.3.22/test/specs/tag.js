  ...
  it('The `exclude` option to ignore parts of the tag', function() {
    var parts = compiler.compile(cat(fixtures, 'treeview.tag'), {
      ...
      exclude: ['html', 'js']
        ...
        dummyTag = ['<my-tag>', '<p>{ hi }</p>', '<style scoped>', ' :scope { color: red; }', '</style>', 'this.hi = "hi"', '</my-tag>'].join('\n');
    expect(compileStr(dummyTag, '', {exclude: ['html']})).to.be(norm("riot.tag2('my-tag', '', 'my-tag,[riot-tag=\"my-tag\"] { color: red; }', '', function(opts) {\nthis.hi = \"hi\"\n});"));
    expect(compileStr(dummyTag, '', {exclude: ['html', 'js']})).to.be(norm("riot.tag2('my-tag', '', 'my-tag,[riot-tag=\"my-tag\"] { color: red; }', '', function(opts) {\n});"));
    expect(compileStr(dummyTag, '', {exclude: ['css']})).to.be(norm("riot.tag2('my-tag', '<p>{hi}</p>', '', '', function(opts) {\nthis.hi = \"hi\"\n}, '{ }');"));
    ...
    parts = compiler.compile(cat(fixtures, 'oneline.tag'), {
      entities: true,
      exclude: ['html']
    });
...
describe('The (internal) `brackets.array` function', function() {
  ...
  it('creation of regexes is not slow, Garbage Collection will be.', function() {
    ...
    for (i = 0; i < CNT; i++)
      arrayFn(bp[i % 1]);
    t2 = Date.now() - t2;
    console.log('\t%d calls using cache: %d ms, creating regexes: %d ms', CNT, t1, t2);
  });
